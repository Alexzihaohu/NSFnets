...
class VPNSFnet:
    ...
    def __init__(self, xb, yb, ub, vb, x, y, layers):
        ...
        self.optimizer_Adam = tf.train.AdamOptimizer(learning_rate=self.learning_rate,
                                                                beta1=0.9,
                                                                beta2=0.999,
                                                                epsilon=1e-08)# add learning rate here
        self.train_op_Adam = self.optimizer_Adam.minimize(self.loss)

        self.optimizer_GD = tf.train.GradientDescentOptimizer(learning_rate=self.learning_rate)
        self.train_op_GD = self.optimizer_GD.minimize(self.loss)

        self.optimizer_MO = tf.train.MomentumOptimizer(self.learning_rate, 0.9)
        self.train_op_MO = self.optimizer_MO.minimize(self.loss)

        self.optimizer_Adagrad = tf.train.AdagradientOptimizer(learning_rate=self.learning_rate)
        self.train_op_Adagrad = self.optimizer_Adagrad.minimize(self.loss)

        self.optimizer_RMSProp = tf.train.RMSPropOptimizer(0.001, 0.9)
        self.train_op_RMSProp = self.optimizer_RMSProp.minimize(self.loss)
...
# train
    ...
    def GD_train(self, nIter=5000, learning_rate=1e-3):
        ...
        start_time = time.time()
        for it in range(nIter):
            self.sess.run(self.train_op_GD, tf_dict)
    ...
    def MO_train(self, nIter=5000, learning_rate=1e-3):
        ...
        start_time = time.time()
        for it in range(nIter):
            self.sess.run(self.train_op_MO, tf_dict)
    ...
    def Adagrad_train(self, nIter=5000, learning_rate=1e-3):
        ...
        start_time = time.time()
        for it in range(nIter):
            self.sess.run(self.train_op_Adagrad, tf_dict)
    ...
    def RMSProp_train(self, nIter=5000):
        ...
        start_time = time.time()
        for it in range(nIter):
            self.sess.run(self.train_op_RMSProp, tf_dict)
...
# 不需要改变 可能需要注意x_tf等
    ...
    def saver(self, string):
        self.save.save(self.sess, 'ckpt/'+string, gobal_step=0)
